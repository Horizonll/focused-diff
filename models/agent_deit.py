...
class AgentAttention(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.dwc = nn.Conv2d(
            in_channels=dim, out_channels=dim, kernel_size=(3, 3), padding=1, groups=dim
        )
        ...
        trunc_normal_(self.ac_bias, std=0.02)
        trunc_normal_(self.ca_bias, std=0.02)
    ...
    def forward(self, x):
        ...
        position_bias1 = position_bias1.reshape(
            1, num_heads, self.agent_num, -1
        ).repeat(b, 1, 1, 1)
        position_bias2 = (
            (self.ah_bias + self.aw_bias)
            .reshape(1, num_heads, self.agent_num, -1)
            .repeat(b, 1, 1, 1)
        )
        position_bias = position_bias1 + position_bias2
        position_bias = torch.cat(
            [self.ac_bias.repeat(b, 1, 1, 1), position_bias], dim=-1
        )
        ...
        agent_bias1 = (
            agent_bias1.reshape(1, num_heads, self.agent_num, -1)
            .permute(0, 1, 3, 2)
            .repeat(b, 1, 1, 1)
        )
        agent_bias2 = (
            (self.ha_bias + self.wa_bias)
            .reshape(1, num_heads, -1, self.agent_num)
            .repeat(b, 1, 1, 1)
        )
        ...
        x[:, 1:, :] = x[:, 1:, :] + self.dwc(v_).permute(0, 2, 3, 1).reshape(
            b, n - 1, c
        )
...
def agent_deit_base(pretrained=False, **kwargs):
    ...
    """
    model_kwargs = dict(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, agent_layer=4, **kwargs
    )
    model = _create_vision_transformer(
