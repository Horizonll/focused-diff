...
class AgentAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        q, k, v = qkv.unbind(0)
        agent_tokens = (
            self.pool(q.reshape(B, H, W, C).permute(0, 3, 1, 2))
            .reshape(B, C, -1)
            .permute(0, 2, 1)
        )
        ...
        agent_tokens_neg = F.elu(agent_tokens_neg) + 1
        k = F.elu(k) + 1
        ...
        agent_tokens_pos = F.elu(agent_tokens_pos) + 1
        q_neg = F.elu(q_neg) + 1
    ...
    def __init__(
        ...
        attn_types = [
            (
                attn_type
                if attn_type[0] != "M"
                else ("A" if i < int(attn_type[1:]) else "B")
            )
            for i in range(depth)
        ]
        window_sizes = [
            (window_size if attn_types[i] == "A" else max(7, (window_size // 8)))
            for i in range(depth)
        ]
        self.blocks = nn.ModuleList(
            [
                SwinTransformerBlock(
                    ...
                    block_depth=block_depth_base + i,
                )
                for i in range(depth)
            ]
        )
        ...
        **kwargs,
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(
                ...
                block_depth_base=sum(depths[:i_layer]),
            )
