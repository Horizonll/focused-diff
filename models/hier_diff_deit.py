...
class AgentAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        attn = self.attn_drop(attn)  # attn_drop
        v_hat_pos = attn @ v
        ...
        attn_neg1 = self.softmax(q1 * (self.scale) @ v_hat_neg1.transpose(-2, -1))
        attn_neg2 = self.softmax(q2 * (self.scale) @ v_hat_neg2.transpose(-2, -1))
        ...
        x[:, 1:, :] = x[:, 1:, :] + self.dwc(v).permute(0, 2, 3, 1).reshape(B, N - 1, C)
